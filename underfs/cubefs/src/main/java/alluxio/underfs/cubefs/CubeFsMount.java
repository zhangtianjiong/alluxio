...
public class CubeFsMount {
    ...
    private long cid;
    private static final Logger LOGGER = LoggerFactory.getLogger(CubeFsUnderFileSystem.class);
...
//        }
    ...
    public int rmdir(String path, boolean isDir) throws IOException {
        int r = libcfs.cfs_rmdir(cid, path, isDir);
//        if (r < 0) {
//            throw error(r, path);
//        }
        return r;
    ...
    public int listDir(int fd, CubeFsLib.DirentInfo[] direntInfos, int batchSize) {
        Pointer arr = direntInfos[0].getPointer();
        CubeFsLib.DirentArray.ByValue slice = new CubeFsLib.DirentArray.ByValue();
        slice.data = arr;
        slice.len = batchSize;
        slice.cap = batchSize;
        long arrSize = libcfs.cfs_lsdir(this.cid, fd, slice, batchSize);
        if (arrSize > 0) {
            for (int i = 0; i < (int) arrSize; i++) {
                direntInfos[i].read();
            }
